import TEXICLO from "./texiclo.svg";
// import LOGO from "./banner1.svg";
import LOGO from "./banner1.jpg";
import LOGO1 from "./banner2.svg";
import LOGO2 from "./banner3.svg";
import HEROIMAGE from "./hero.svg";
import IDEAIMAGE from "./Idea.svg"; 
import QUALITYICON from "./quality.svg";
import MATCHICON from "./mix.svg";
import SHIPPINGICON from "./world.svg";
import CHECKICON from "./yellow.svg";
import NEWCOLLECTION from "./newCollection.svg";
import WOMENS from "./womens.svg";
import KIDS from "./kids.svg";
import MENS from "./mens.svg";
import POLO from "./polo.svg";
import SWEATSHIRTS from "./sweatshirt.svg";
import HOODIE from "./hoodie.svg";
import STEP1 from "./step1.svg";
import STEP2 from "./step2.svg";
import STEP3 from "./step3.svg";
import TSHIRT from "./tshirt.svg";
import CONTACT from "./contact.png";
import CROPTOP from "./croptop.svg";

import FRONTIMG from "./frontimg.svg";
import BACKIMG from "./backimg.svg";
import SLEEVEIMG from "./sleeveimg.svg";




export {
    TEXICLO,LOGO,LOGO1,LOGO2,HEROIMAGE,IDEAIMAGE,QUALITYICON,MATCHICON,SHIPPINGICON,CHECKICON,
    NEWCOLLECTION,WOMENS,MENS,KIDS,POLO,HOODIE,SWEATSHIRTS,TSHIRT,STEP1,STEP2,STEP3,CONTACT,CROPTOP,FRONTIMG,BACKIMG,SLEEVEIMG
}



